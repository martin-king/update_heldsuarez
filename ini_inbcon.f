...
cmpk      read (20) ((r4inp(i,j),i=1,ix),j=il,1,-1)
      do j = 1,il
        do i = 1,ix
cmpk          phi0(i,j) = grav0*r4inp(i,j)
          phi0(i,j)=grav0*1000*abs(sin(dble(i)/dble(ix)*6.28))
        enddo
      enddo
...
C--   2.3 Land-surface temp.
      ...
      do it = 1,12
cmpk        read (23) ((r4inp(i,j),i=1,ix),j=il,1,-1)
        do j = 1,il
          do i = 1,ix
cmpk            stl12(i,j,it) = r4inp(i,j)
             stl12(i,j,it)=293.d0
          enddo
        enddo
      enddo
...
C--   2.4 Snow depth
      ...
      do it = 1,12
cmpk        read (24) ((r4inp(i,j),i=1,ix),j=il,1,-1)
        do j = 1,il
          do i = 1,ix
cmpk            snowd12(i,j,it) = r4inp(i,j)
            snowd12(i,j,it)=0.d0
         enddo
        enddo
      enddo
...
cmpk      read (25) ((veg(i,j),i=1,ix),j=il,1,-1)

      do j = 1,il
        do i = 1,ix
cmpk          veg(i,j)=max(0.,0.01*veg(i,j))
          veg(i,j)=0.d0
        enddo
      enddo
...
cmpk        read (26) ((swl1(i,j),i=1,ix),j=il,1,-1)
cmpk        read (26) ((swl2(i,j),i=1,ix),j=il,1,-1)
cmpk        read (26) dummy4

        do j = 1,il
          do i = 1,ix
cmpk            swroot = idep2*swl2(i,j)
cmpk            soilw12(i,j,it) = min(1.,rsw*(swl1(i,j)+
cmpk     &                        veg(i,j)*max(0.,swroot-swwil2)))		
             swroot=0.d0
             soilw12(i,j,it)=0.d0                         
          enddo
        enddo
...
C--   3.2 SST 
      ...
      do it = 1,12
cmpk        read (21) ((r4inp(i,j),i=1,ix),j=il,1,-1)
        do j = 1,il
          do i = 1,ix
cmpk            sst12(i,j,it) = r4inp(i,j)
           sst12(i,j,it)=293.d0
          enddo
        enddo
      enddo
...
c     3.2 Sea ice fraction
      ...
      do it = 1,12
cmpk        read (22) ((r4inp(i,j),i=1,ix),j=il,1,-1)
        do j = 1,il
          do i = 1,ix
cmpk            sice12(i,j,it) = r4inp(i,j)
           sice12(i,j,it)=0.d0
          enddo
        enddo
      enddo
...
cmpk          if (it.ne.2.or.isst0.gt.1)
cmpk     &       read (30) ((r4inp(i,j),i=1,ix),j=il,1,-1)

          do j = 1,il
            do i = 1,ix
cmpk              sstan3(i,j,it) = r4inp(i,j)
              sstan3(i,j,it) = 0.d0
            enddo
          enddo
...
cmpk        open ( unit=31, file='fort.31', status='old', 
cmpk     &         form='unformatted', access='direct', recl=irecl )

        do it = 1,12

          irec=irec+2
cmpk          read (31,rec=irec) r4inp

          do j = 1,il
            do i = 1,ix
cmpk              hfseacl(i,j) = hfseacl(i,j)+r4inp(i,j)
              hfseacl(i,j) = 0.d0
            enddo
          enddo  
...
cmpk                hfseacl(i,j) = hfseacl(i,j)/(12.*fmask_s(i,j))
                 hfseacl(i,j) = 0.d0
            else
                hfseacl(i,j) = 0.
            endif
...
cmpk           read (32) r4inp
          ...
          do j = 1,il
            do i = 1,ix
cmpk              sstom12(i,j,it) = sst12(i,j,it)+r4inp(i,j)
             sstom12(i,j,it) = 293.d0
            enddo
          enddo  
